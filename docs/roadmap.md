...
|---|---|---|---|
| M0 Foundation | **In progress** | Cargo workspace; versioned domain contracts; UUIDv7/UTC/hash invariants; SQLite hash-chained event log; JSON CLI. `cargo fmt --all`, `cargo test --workspace` (53 tests) and `cargo clippy --workspace --all-targets -- -D warnings` all pass. | Add property-style schema invariant tests. |
