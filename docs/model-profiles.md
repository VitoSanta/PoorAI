...
The gate is on presence, and the **rate now shapes the run** rather than being rounded away. A deployment observed at 2 of 3 on structural calls is not one that cannot emit them: a miss is a coin flip, and three consecutive misses at that rate happens about once in twenty-seven runs, so the malformed-call limit scales with the measured rate — bounded, since an unbounded retry is a run that never ends. One measured reliable keeps the original limit, because widening it for a deployment that always emits spends budget on a deployment that has no trouble. The rates and the limit they produced are recorded in `run.started`.

That is the narrow version. A per-deployment retry policy, a tool schema narrowed for a deployment that struggles with the full one, and reasoning escalation when a task proves hard are still not built.
